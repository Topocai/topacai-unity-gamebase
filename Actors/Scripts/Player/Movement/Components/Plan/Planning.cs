...
namespace Topacai.Player.Movement.Components
{
    public class Planning : MovementComponent
    {
        ...
        private void Update()
        {
            _canPlan = !InConflict(_incompatibleStates) && _delayTimer < 0f && _movement.IsFalling;

            bool newPlanning = _movement.PlayerBrain.InputHandler.GetActionHandler(ActionName.Jump).All && _canPlan;
